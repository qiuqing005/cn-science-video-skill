...
素材与字幕合流后先执行视觉门禁：每条素材必须绑定对应旁白原句、语义作用和相关性理由，相关性评分至少 0.7；构图签名至少 75% 不同，实拍友好主题至少 3 个场景具有不同动态视频，同一视频不得跨场复用，已用素材必须具备标题、来源和许可证字段。随后才执行 HyperFrames 检查。严格质量门禁成功后写 `work/quality-checks.ok`；组合及检查规则未变化时不重复启动 Chrome。最终媒体验收成功后写 `work/final-media.ok.json`，其中保存文件大小与 SHA-256；只有哈希仍一致时才复用 ffprobe、黑帧和响度结论。
