...
- 120 秒以内可复用时间线、字幕、媒体装载和验收框架，但不得复用整套场景构图后只替换标题。每段必须声明不同的 `visual_type` 和 `composition_signature`，并针对核心机制设计不同的主视觉。
- 实拍友好的主题至少使用 3 段高关联动态视频并覆盖至少 3 个场景；同一视频不得跨场景重复充当背景。静态图片只作为证据、剖面或确实找不到视频时的例外，不能默认铺满整段。
- 素材必须在镜头级绑定旁白原句，而不是只与整段主题宽泛相关。每条已用素材记录 `shot_id`、`narration_span`、`semantic_role`、`relevance_reason` 和 `relevance_score`；优先直接展示旁白中的对象、动作、机制、尺度、证据或结果，不能用“科技感”“运动感”等气氛素材代替语义证据。
- 动画优先设计为 2-5 秒的机制演示脉冲；装饰动画不做无意义的全程循环。静态帧复用率只用于观察渲染性能，不得作为创作质量目标，也不得以提高复用率为由冻结应当运动的画面。
...
- 现有模型不能满足语言、音色或稳定性时，AI 可以自主从 Hugging Face、ModelScope 或项目官方仓库评估并下载 TTS 模型。维护状态优先于参数量和热度：优先官方仓库未归档、近 12 个月有实质提交且安装文档仍适配当前环境的项目。再核对许可证、模型大小、当前 GPU/CPU 适配和推理依赖；优先官方权重和 safetensors，拒绝不明网盘、任意可执行安装包、停止维护的未知 fork 与未审查的远程代码。
...
4. 先执行视觉门禁，检查旁白原句绑定、语义作用、相关性评分、场景构图签名、动态视频覆盖、跨场素材重复和授权字段；再检查普通话、断句、音量与语气一致性，并执行 `lint`、`validate`、`inspect` 及中点快照/接触表。
